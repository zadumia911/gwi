...
@section('title','Bank Deposit Manage')
@section('content')
      ...
      <div class="row">
        <div class="col-sm-6">
          <div class="page-title">
            <h6>Bank Deposit Manage</h6>
          </div>
        </div>
        <div class="col-sm-6">
          <div class="short-icon text-right">
            <a href="{{url('admin/deposit/add')}}" class="btn btn-primary"><i class="fas fa-plus"></i></a>
          </div>
        </div>
      </div>
    </div>
          ...
          <tr>
            ...
            <td class="action_button">
              <ul class="manage-btn-group">
                <li><a href="{{url('admin/deposit/edit/'.$value->id)}}" class="btn btn-info"><i class="fa fa-edit"></i></a></li>
                <li>@if($value->status==1)
                  <form action="{{url('admin/deposit/inactive')}}" method="POST">
                      @csrf
                ...
                @else
                <form action="{{url('admin/deposit/active')}}" method="POST">
                  @csrf
                  <input type="hidden" name="hidden_id" value="{{$value->id}}">
                ...
                @endif
                <li>
                  <form action="{{url('admin/deposit/delete')}}" method="POST">
                    @csrf
                    <input type="hidden" name="hidden_id" value="{{$value->id}}">
