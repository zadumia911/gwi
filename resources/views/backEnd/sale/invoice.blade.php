    ...
    <br>
     <div class="row invoice-info" style="margin-top: -20px">
      ...
      <div class="col-sm-4 invoice-col"> 
        ...
        <div style="background: #E1E5F3;border-radius: 18px;padding: 10px;font-size:23px">
         Name : {{$invoice->customer->customer_name}}<br>
        <span style="font-size:21px"> Address : {{$invoice->customer->customer_address}}</span><br>
       <span style="font-size:21px">  Mobile : {{$invoice->customer->customer_phone}}</span><br>
       <span style="font-size:21px">  Email : {{$invoice->customer->customer_email}}</span><br>
       <span style="font-size:21px">  Web : {{$invoice->customer->customer_web}}</span><br>
...
<div class="container-fluid" id="fb">
        ...
        <table style="width: 100%;font-size:19px !important;border-radius:50px" border="1" cellpadding="5"  cellspacing="5" >
         ...
         <tbody>
          <tr>
            ...
            <td class="text-center" style="font-size:19px !important">{{$invoice->date}}</td>
            <td class="text-center" style="font-size:19px !important">{{$invoice->bill}}</td>
            <td class="text-center" style="font-size:19px !important">{{$invoice->date}}</td>
            <td class="text-center" style="font-size:19px !important">{{$invoice->payment_method}}</td>
              <td class="text-right"  style="font-size:19px !important;color:red;font-weight:650"><stonrg><span style="float:left;">TK</span> {{$invoice->due}}</stonrg></td>
          </tr>
         </tbody>
        </table>
      </div>
    ...
    <tbody style="border-bottom:1px solid !important;border-left:1px solid !important;border-right:1px solid !important">
      @foreach($invoice->saledetails as $value)
      <tr >
