      ...
      <table id="example1" class="table table-bordered table-responsive">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{$value->supplier_web}}</td>
            <td>{{$value->supplier_balance}}</td>
            <td>{{$value->status ==1 ? 'Active' : 'Inactive'}}</td>
            <td class="action_button">
              <ul class="manage-btn-group">
                <li><a href="{{url('editor/supplier/edit/'.$value->id)}}" class="btn btn-info"><i class="fa fa-edit"></i></a></li>
                <li>@if($value->status==1)
                  <form action="{{url('editor/supplier/inactive')}}" method="POST">
                      @csrf
                ...
                @else
                <form action="{{url('editor/supplier/active')}}" method="POST">
                  @csrf
                  <input type="hidden" name="hidden_id" value="{{$value->id}}">
                ...
                @endif
                 <li>
                  <form action="{{url('editor/supplier/delete')}}" method="POST">
                    @csrf
                    <input type="hidden" name="hidden_id" value="{{$value->id}}">
