...
@section('title','Local Cost Head Manage')
@section('content')
      ...
      <div class="row">
        <div class="col-sm-6">
          <div class="page-title">
            <h6>Local Cost Head Manage</h6>
          </div>
        </div>
        <div class="col-sm-6">
          <div class="short-icon text-right">
            <a href="{{url('editor/localcosthead/add')}}" class="btn btn-primary"><i class="fas fa-plus"></i></a>
          </div>
        </div>
      </div>
    </div>
        ...
        <tbody>
          ...
          <tr>
            <td>{{$loop->iteration}}</td>
             <td>{{$value->local_cost_head}}</td>
            <td>{{$value->status==1 ? 'Active' : 'Inactive'}}</td>
            <td class="action_button">
              <ul class="manage-btn-group">
                <li><a href="{{url('editor/localcosthead/edit/'.$value->id)}}" class="btn btn-info"><i class="fa fa-edit"></i></a></li>
                <li>
                @if($value->status==1)
                  <form action="{{url('editor/localcosthead/inactive')}}" method="POST">
                      @csrf
                ...
                @else
                 <form action="{{url('editor/localcosthead/active')}}" method="POST">
                  @csrf
                   <input type="hidden" name="hidden_id" value="{{$value->id}}">
                ...
                @endif
                 <li>
                  <form action="{{url('editor/localcosthead/delete')}}" method="POST">
                    @csrf
                    <input type="hidden" name="hidden_id" value="{{$value->id}}">
